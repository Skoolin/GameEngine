...
public class InsertionSort {
    ...
    public static void sortHighToLowS(List<ParticleSystem> list) {
        for (int i = 1; i < list.size(); i++) {
        	ParticleSystem item = list.get(i);
            if (item.getCameraDistSqr() > list.get(i - 1).getCameraDistSqr()) {
                sortUpHighToLowS(list, i);
            }
        }
    }
 
    private static void sortUpHighToLowS(List<ParticleSystem> list, int i) {
    	ParticleSystem item = list.get(i);
        int attemptPos = i - 1;
        while (attemptPos != 0 && list.get(attemptPos - 1).getCameraDistSqr() < item.getCameraDistSqr()) {
            attemptPos--;
        }
        list.remove(i);
        list.add(attemptPos, item);
