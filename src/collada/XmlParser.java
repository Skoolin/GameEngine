...
public class XmlParser {
	 ...
	 * allowing for easy access to the data contained in the XML file.
	 * 
	 * @param fileAdress - the XML file
	 * @return The root node of the XML structure.
	 */
	public static XmlNode loadXmlFile(String fileAdress) {
		BufferedReader reader = null;
		try {
			FileReader isr = new FileReader(new File(fileAdress));
			reader = new BufferedReader(isr);
		} catch (Exception e) {
			e.printStackTrace();
			System.err.println("Can't find the XML file: " + fileAdress);
			System.exit(0);
			return null;
		}
		try {
			...
			e.printStackTrace();
			System.err.println("Error with XML file format for: " + fileAdress);
			System.exit(0);
			return null;
		}
	}
